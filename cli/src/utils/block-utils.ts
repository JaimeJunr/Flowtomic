...
import { resolveComponentPath } from "./resolve-repo";
...
export async function addBlock(
  ...
  if (block.registryDependencies && block.registryDependencies.length > 0) {
    console.log(
      chalk.blue(`   📋 Dependências do registry: ${block.registryDependencies.join(", ")}`)
    );
...
async function processBlockFile(
  ...
  const sourcePath = resolveComponentPath(join("packages/ui/src/blocks", file.path), repoPath);
