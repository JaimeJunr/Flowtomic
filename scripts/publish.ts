 ...
 */
function updateVersion(packagePath: string, versionType: VersionType): string {
  const packageJsonPath = join(process.cwd(), packagePath, "package.json");
  const packageJson = JSON.parse(readFileSync(packageJsonPath, "utf-8"));
 ...
 * Executa testes
 */
async function runTests(packageName: string): Promise<void> {
  console.log("🧪 Executando testes...");
  const result = await $`CI=true bun run test --filter=${packageName} -- --run`;
...
async function publishToNpm(packagePath: string): Promise<void> {
  ...
  const result = await $`cd ${packagePath} && npm publish --access public`;
  if (result.exitCode !== 0) {
...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "--package" || arg === "-p") {
      ...
      !arg.startsWith("-") &&
      !["major", "minor", "patch", "ui", "logic", "cli"].includes(arg)
    ) {
  ...
  try {
    // 1. Executar testes
    await runTests(packageInfo.name);

    // 2. Executar build
    await runBuild(packageInfo.name);
