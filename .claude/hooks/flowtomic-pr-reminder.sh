#!/usr/bin/env bash
# flowtomic: lembrete de realimentar o CLAUDE.md do repo ao abrir/atualizar um PR.
#
...
case "$cmd" in
  *"git push"*)
    msg="flowtomic — checkpoint de PR: se esta tarefa revelou um gotcha, mudou um comando/stack ou firmou uma convencao, atualize o CLAUDE.md da raiz ANTES de fechar (secoes Armadilhas, Comandos, Stack, Convencoes). Siga o \"Checklist ao abrir PR\" de la. Se nada novo surgiu, ignore."
    if command -v jq >/dev/null 2>&1; then
