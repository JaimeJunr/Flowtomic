#!/usr/bin/env bash
# flowtomic: mantem o contexto atualizado com o time no INICIO da sessao (SEGURO).
#
...
$TO git fetch --quiet 2>/dev/null || {
  echo "flowtomic: sem rede pra checar atualizacoes do time (seguindo offline)."
  exit 0
}
...
if [ "$local_sha" = "$base_sha" ]; then
  if [ -z "$(git status --porcelain)" ]; then
    if git merge --ff-only --quiet '@{u}' 2>/dev/null; then
      echo "flowtomic: atualizado com o time (+${behind} commit(s), fast-forward)."
    else
      echo "flowtomic: ha +${behind} commit(s) do time. Rode 'git pull' pra atualizar."
    fi
  else
    echo "flowtomic: ha +${behind} commit(s) do time, mas voce tem mudancas nao-commitadas — nao puxei pra nao sobrescrever. Rode 'git pull' quando quiser."
  fi
elif [ "$remote_sha" = "$base_sha" ]; then
  echo "flowtomic: voce tem ${ahead} commit(s) local(is) nao publicado(s) — 'git push' quando quiser compartilhar com o time."
else
  echo "flowtomic: local e time divergiram (${ahead} local x ${behind} do time). Reconcilie com 'git pull'."
fi
exit 0
