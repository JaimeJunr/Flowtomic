...
async function loadIndex() {
	...
	if (!r.ok) throw new Error(`index.json respondeu ${r.status} — o Storybook está no ar em ${BASE}?`);
		...
		if (cmd === "smoke") {
			...
			for (const s of stories) {
				...
				try {
					...
					if (!rendered.ok) {
						await p.screenshot({ path: path.join(SHOTS, `FALHA-${s.id}.png`), animations: "disabled" });
