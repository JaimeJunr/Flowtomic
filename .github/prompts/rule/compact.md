# Comando /compact - Compactação Inteligente de Regras

> **🎯 Objetivo**: Compactar arquivos priorizando informações mais recentes, removendo dados obsoletos e mantendo funcionalidade crítica. Integra-se ao sistema do agente fornecendo boas práticas e funcionalidades de compactação de arquivos.

## 1. Contexto e Preparação

- [ ] **SEMPRE siga rigorosamente** as diretrizes do [formatter-rule-core.mdc](formatter-rule-core.mdc) em todas as fases do processo de compactação, garantindo padronização e qualidade em cada etapa.
- [ ] **NUNCA ignore** ou adapte arbitrariamente os padrões definidos
- [ ] **SEMPRE revise** as regras do formatter antes de iniciar o processo
- [ ] **SEMPRE obtenha** data atual do sistema: `date +"%Y-%m-%d %H:%M:%S"`
- [ ] **SEMPRE leia** o arquivo completo para entender estrutura e conteúdo
- [ ] **SEMPRE identifique** tipo de regra (alwaysApply, globs específicos, evolution)
...
- [ ] **SEMPRE preserve** frontmatter (`---` e metadados) durante compactação
...
**Automação**: Configure ferramentas para validação automática de frontmatter

### 2.3 Adaptação por Tipo de Regra

#### 2.3.1 Regras Always Apply (Aplicação Universal)

- [ ] **SEMPRE mantenha** princípios fundamentais
- [ ] **SEMPRE elimine** exemplos muito específicos
- [ ] **SEMPRE preserve** diretrizes universais
- [ ] **SEMPRE consolide** seções similares

**Características**: `alwaysApply: true`, foco em princípios fundamentais, linguagem genérica

#### 2.3.2 Regras com Globs Específicos

- [ ] **SEMPRE mantenha** comandos específicos da tecnologia
- [ ] **SEMPRE preserve** exemplos de código relevantes
- [ ] **SEMPRE consolide** padrões similares
- [ ] **SEMPRE atualize** ferramentas para versões atuais

**Características**: `globs: *.py, *.js, *.ts`, foco em tecnologia específica, comandos específicos

#### 2.3.3 Arquivos Evolution.mdc

- [ ] **SEMPRE priorize** padrões mais recentes
- [ ] **SEMPRE consolide** padrões duplicados
- [ ] **SEMPRE mantenha** lições aprendidas essenciais
- [ ] **SEMPRE atualize** aplicações futuras por relevância

**Características**: Padrões comprovados, lições aprendidas, estrutura consolidada

#### 2.3.4 Arquivos de Contexto

- [ ] **SEMPRE leia** todos os arquivos de contexto relevantes
- [ ] **SEMPRE identifique** informações obsoletas
- [ ] **SEMPRE preserve** estrutura hierárquica
- [ ] **SEMPRE mantenha** integração com modos

**Características**: Informações temporárias e estruturais, integração com modos
...
#### 5.1.1 Erro: Frontmatter Corrompido

**Sintomas**: Arquivo não carrega corretamente, metadados perdidos

**Solução**:

- [ ] **SEMPRE verifique** estrutura `---` no início e fim
- [ ] **SEMPRE restaure** backup antes de continuar
- [ ] **SEMPRE valide** sintaxe YAML do frontmatter
- [ ] **SEMPRE teste** carregamento após correção

```bash
# Verificar estrutura do frontmatter
head -10 target_file.mdc | grep -E "^-{3}$"

# Validar sintaxe YAML
head -10 target_file.mdc | tail -n +2 | head -n -1 | yamllint
```
...
# Corrigir erros automaticamente
markdownlint-cli2 target_file.mdc --fix --config .markdownlint.json
```
...
### 6.1 Compactar Arquivos de Contexto

```bash
/compact context
# Aplica estratégia específica para arquivos de contexto
```

### 6.2 Compactar Evolution

```bash
/compact evolution
# Aplica estratégia específica para evolution.mdc
```

### 6.3 Compactar Regra Específica

```bash
/compact target_file.mdc
# Aplica estratégia baseada no tipo de regra identificado
```
...
/compact target_file.mdc --validate --format

# Compactar com configuração específica
/compact target_file.mdc --config .compact.json

# Compactar com relatório detalhado
/compact target_file.mdc --report --verbose
```
...
- [formatter-rule-core.mdc](formatter-rule-core.mdc) - Padrões de formatação
